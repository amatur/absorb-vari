...
public:
    ...
    typedef tuple<unitig_t,unitig_t,int, int> fourtuple; // uid, walkid, pos, isTip
...
//                        sinkSrcEdges[u] = myvector;
        ...
        if(ALGOMODE==BRACKETCOMP){
            ...
            for (int sinksrc = 0; sinksrc<V_bcalm; sinksrc++) {
    ...
    void run(string graph_file_name, int K, bool runFlag = 0){
       ...
       // decodeTip(K, "fa.esstip", "fa.spss.fa");
         if(param.VALIDATE){
             ...
             decodeTip(K,param.ofileTipOutput);
         ...
         float ustitchCharPerKmer = stat.C_esstip*1.0/stat.nKmers;
        
         
         stat.statPrinter(globalStatFile, "V_ESSTIP", stat.V_esstip);
         stat.statPrinter(globalStatFile, "C_ESSTIP", stat.C_esstip);
         stat.statPrinter(globalStatFile, "CHAR_PER_KMER_ESSTIP", ustitchCharPerKmer,true);
        ...
        cout << "------------ UST completed successfully. Output is in file "<<param.OUTPUT_FILENAME << " ------------"<<endl;
         ...
         cout << "Lower bound on any SPSS representation " <<  "= " << (stat.charLowerbound)*1.0/stat.nKmers << " char/k-mer"<< endl;
         
         cout << "Size of ESS TIP output" <<  "= " <<ustitchCharPerKmer << " char/k-mer"<< endl;
