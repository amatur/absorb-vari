...
void ESS::readUnitigFile(const string& unitigFileName, vector<unitig_struct_t>& unitigs, vector<vector<edge_t> >& adjList)
{
    ...
    FLG_ABUNDANCE = false;
    bool abundanceFlgDetermine = false;
...
void ESS::collectInput(int argc, char** argv, string & graph_file_name, int & K, bool & IS_TIP){
    char * nvalue;
    char c;
            ...
            case 't':
                if(optarg) {
                    if(strcmp(optarg, "0")==0 || strcmp(optarg, "1")==0){
                        IS_TIP = static_cast<bool>(std::atoi(optarg));
                    }else{
...
void ESS::indegreePopulateAndLowerBound(){
...
public:
    ...
    void iterSpellEfficient(int startWalkIndex2, vector<char>& color, ofstream& nostrfile);
...
void AbsorbGraph::run(string graph_file_name, int K, bool runFlag){
    ...
    globalStatFile.open((getFileName(graph_file_name)+".stats.txt").c_str(), std::fstream::out);
    ...
    float percent_saved_c = (1-(stat.C_ess*1.0/stat.C_bcalm))*1.0;
    float ustitchBitsPerKmer = stat.C_ess*2.0/stat.nKmers;
    ...
    cout << "------------ ESS completed successfully. Output is in file "<<param.OUTPUT_FILENAME << " ------------"<<endl;
...
vector<int> AbsorbGraph::getAllUidsInWalk(int walkId){  //not  tested
    vector<int> uids;
...
void AbsorbGraph::iterSpellEfficient(int startWalkIndex2, vector<char>& color, ofstream &tipNoStrFile)
{
...
void AbsorbGraph::makeGraphDotAbsorb(map<int, vector<edge_t> > adjList){
    FILE * fp;
...
{
    //vector<threetuple>& sorter, map<int, int >& sorterIndexMap
    int countSCC = 0;
